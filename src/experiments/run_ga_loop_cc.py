...
def _fitness_from_delta(delta: np.ndarray, target_classes: List[int]) -> float:
    K = int(delta.shape[0])
...
def main() -> None:
    ...
    # CC needs BOTH ckpt_e and the original ckpt_{e+1}
    ap.add_argument("--ckpt_e", type=str, required=True, help="checkpoint at epoch e (start point for TRAINONEEPOCH)")
    ap.add_argument("--ckpt_orig_e1", type=str, required=True, help="original checkpoint at epoch e+1 (unweighted)")
    ap.add_argument("--P_train", type=str, required=True, help="P_train.npy (N x K) computed at epoch e")
    ap.add_argument("--targets", type=str, required=True, help='target classes, e.g. "5,7,9"')

    ap.add_argument("--out_dir", type=str, required=True)

    ap.add_argument("--train_aug", type=int, default=0)
    ...
    # CC: baseline for delta is the ORIGINAL epoch e+1 checkpoint
    orig_e1_model = ResNet9(num_classes=K).to(args.device)
    ...
    for g in range(int(args.gens)):
        ...
        for i, alpha in enumerate(population):
            ...
            # Algorithm 1 line 5: train one epoch from theta_e
            ...
            acc_new_e1 = np.array(after_metrics["per_class_acc"], dtype=np.float32)

            # CC delta: original epoch e+1 vs newly weighted epoch e+1
            delta = _compute_delta_pct(acc_before=acc_orig_e1, acc_after=acc_new_e1)
            ...
            rec = {
                ...
                "acc_orig_e1": acc_orig_e1.tolist(),
                "acc_new_e1": acc_new_e1.tolist(),
